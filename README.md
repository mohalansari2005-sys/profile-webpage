...
`next.config.ts` sets `output: "export"`, so `next build` pre-renders every route
...
- `components/sections/work.tsx` — holds the join's state
- `components/reveal.tsx` — needs DOM geometry and IntersectionObserver
...
The palette is defined once as custom properties in `app/globals.css` and mapped
...
`components/reveal.tsx` does **not** rely on IntersectionObserver alone, and
...
`AGENTS.md` is generated and rewritten by `next dev` itself, not by hand. It's
expected to reappear if deleted; commit it rather than fight it.

`public/*.svg` are unused `create-next-app` starter assets.
